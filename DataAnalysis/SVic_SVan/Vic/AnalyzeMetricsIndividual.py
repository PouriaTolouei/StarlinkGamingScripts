...
def graphDistr(metrics, xLabel, fileName, minm, maxm, step):
    ...
    plt.plot(set1.keys(), set1.values(), marker='', label='CDF')
    plt.legend(loc="upper left")
    plt.xlabel(xLabel, fontsize=16)
    plt.ylabel('Likelihood of occurrence', fontsize=16)
...
def graphSecondsBar(metrics, ylabel, fileName, minm, maxm, step): 
    ...
    plt.xlim(-1, 60)
    plt.xticks(range(0, 60, 2), fontsize=14)
    plt.ylim(minm, maxm)
    plt.yticks(range(minm, maxm, step), fontsize=14)
    plt.bar(seconds, metrics)
    plt.axvline(12, 0, 1, color='red', linestyle='dashed')
    plt.axvline(27, 0, 1, color='red', linestyle='dashed')
    plt.axvline(42, 0, 1, color='red', linestyle='dashed')
    plt.axvline(57, 0, 1, color='red', linestyle='dashed')
    plt.xlabel("Second", fontsize=16)
    plt.ylabel(ylabel, fontsize=16)
    plt.savefig(fileName + "AtSeconds.svg", format = 'svg', dpi=600)
    plt.clf()
    plt.close()
...
while exists:
    ...
    if exists:
        ...
        graphBoxPlot(totalPacketLosses, NONE, "Total Packet Loss", "TotalPacketLoss", 0, 3600, 200)
        graphDistr(totalPacketLosses, "Total Packet Loss", "TotalPacketLoss", 0, 3600, 200)
        graphSecondsBar(totalPacketLossAtSeconds, "Total Packet Loss", "TotalPacketLoss", 0, 3600, 200)
