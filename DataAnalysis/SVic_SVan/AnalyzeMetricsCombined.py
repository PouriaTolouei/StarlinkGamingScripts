...
def graphBoxPlot(metrics1, metrics2, yLabel, fileName, minm, maxm, step, width, height):
    metrics = {'Victoria Starlink 1': metrics1, 'Victoria Starlink 2': metrics2}
...
def graphDistr(metrics1, metrics2, xLabel, fileName, minm, maxm, step, width, height):
    ...
    plt.plot(set1.keys(), set1.values(), marker='', label='Victoria Starlink 1 CDF', color='red')
    plt.plot(set2.keys(), set2.values(), marker='', label='Victoria Starlink 2 CDF', color='blue')
...
testNum = 1 
folderNames = ['Vic (Panbee)', 'Vic (Pantrig)']
